	...
	public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
		...
		try {
			if (value != null && value.trim().length() > 0) {
				asObject = service.getThemes().get(Integer.parseInt(value));
				return service.getThemes().get(Integer.parseInt(value));
				/*
				 * } catch (NumberFormatException e) { throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Conversion Error", "Not a valid theme.")); }
				 */
			}
		} catch (Exception e) {
			e.printStackTrace();
