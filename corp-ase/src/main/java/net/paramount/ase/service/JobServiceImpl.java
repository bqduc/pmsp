...
public class JobServiceImpl implements JobService {
	...
	public List<JobSchedule> getAllScheduledJobs() {
		List<JobSchedule> fetchedResults = ListUtility.createDataList();
		JobSchedule currentJobSchedule = null;
		try {
			Scheduler scheduler = schedulerFactoryBean.getScheduler();
			...
			for (String groupName : scheduler.getJobGroupNames()) {
				for (JobKey jobKey : scheduler.getJobKeys(GroupMatcher.jobGroupEquals(groupName))) {
					...
					currentJobSchedule = JobSchedule.builder()
					...
					if(isJobRunning(jobKey.getName())){
						currentJobSchedule.setStatus("RUNNING");
					}else{
						currentJobSchedule.setStatus(getJobState(jobKey.getName()));
					}
				}
			}
		} catch (SchedulerException e) {
