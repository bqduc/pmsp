...
@Component//@Stateless
public class DeliveryOrderFacade extends BaseDAO {
	/**
	 * 
	 */
	private static final long serialVersionUID = 4861687724906841786L;
	...
	public List<Partner> findTopNCustomers(int n) {
		List result = em.createNamedQuery("Partner.findByCustomer").setMaxResults(n).getResultList();
	...
	public List<Partner> findTopNSuppliers(int n) {
		List result = em.createNamedQuery("Partner.findBySupplier").setMaxResults(n).getResultList();
	...
	public List<DeliveryOrder> findBySaleId(Integer saleId) {
		List result = em.createNamedQuery("DeliveryOrder.findBySaleOrder").setParameter("id", saleId).getResultList();

		return result;
	}

	public List<DeliveryOrder> findByBackOrder(Integer id) {
		List result = em.createNamedQuery("DeliveryOrder.findByBackOrder").setParameter("id", id).getResultList();

		return result;
	}

	public Long countByBackOrder(Integer id) {
		Long count = (Long) em.createNamedQuery("DeliveryOrder.countByBackOrder").setParameter("id", id).getSingleResult();
	...
	public List<DeliveryOrder> findByPurchaseId(int purchaseId) {
		List result = em.createNamedQuery("DeliveryOrder.findByPurchaseOrder").setParameter("id", purchaseId).getResultList();

		return result;
	}

	public List<DeliveryOrder> findInDelivery() {
		List<DeliveryOrder> inDeliveries = em.createNamedQuery("DeliveryOrder.findInDelivery").setParameter("type", "Purchase").getResultList();
		return inDeliveries;
	}

	public List<DeliveryOrder> findOutDelivery() {
		List<DeliveryOrder> outDeliveries = em.createNamedQuery("DeliveryOrder.findOutDelivery").setParameter("type", "Sale").getResultList();
		return outDeliveries;
	}

	public List<DeliveryOrder> findByPartner(Integer partnerId, String type) {
		List<DeliveryOrder> deliveries = em.createNamedQuery("DeliveryOrder.findByPartner").setParameter("partnerId", partnerId).setParameter("type", type).getResultList();
